...
def get_parser():
    ...
    parser.add_argument("--domain_model_ckpt", required=True)

    parser.add_argument("--base_model_prompt_type", required=True)
    parser.add_argument("--domain_model_prompt_type", required=True)

    # parser.add_argument("--reward_model_ckpt", required=True)
...
def post_process_args(args):
    ...
    base_model_name = args.base_model_ckpt.split("/")[-1]
    domain_model_name = args.domain_model_ckpt.split("/")[-1]
    ...
    # print(f"Found {num_gpus} GPUs: {cuda_devices}")
